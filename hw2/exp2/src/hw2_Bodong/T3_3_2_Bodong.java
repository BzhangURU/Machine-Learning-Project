...
public class T3_3_2_Bodong {
	...
	public static void main(String[] args) {
		...
		try {
			   ...
			   double weight[]=weight_simple_perceptron(data,lines,max_feature_index+2);
			   System.out.print("T3.3.2   weight is ");
			   ...
			   System.out.println("T3.3.2   Accuracy in training set is "+accuracy_train_simple);
			   ...
			   try {
				   ...
				   System.out.println("T3.3.2   Accuracy in testing set is "+accuracy_test_simple);
		...
		for(int i=0;i<lines;i++)
		{
			...
			if((sig>=0&&(double)data[i][columns-1]<0)||(sig<0&&(double)data[i][columns-1]>0))//data!=0
			{
				for(int k=0;k<columns-1;k++)
					weight[k]+=rate*(double)data[i][k]*(double)data[i][columns-1];
				weight[columns-1]+=rate*(double)data[i][columns-1];
				num_updates++;
			}
				
		}
		System.out.println("T3.3.2    Totally "+num_updates+" updates in simple Perceptron Algorithm");
		return weight;
	}
